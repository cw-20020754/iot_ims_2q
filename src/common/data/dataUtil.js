export default {
  ...
  getters: {
    ...
    getLocaleText: (state) => (msgId, localeId) => {
      const findMessage = state.messages.find(
        (msg) => msg.msgId === msgId && msg.localeId === localeId
      );

      return findMessage !== undefined ? findMessage.msg : "";
    },
    codes: (state) => state.codes,
    getCodeText: (state) => (category, value) => {
      const findCode = state.codes.find((code) => code.category === category);

      if (findCode !== undefined) {
        const findItem = findCode.items.find((item) => item.value === value);
        return findItem !== undefined ? findItem.text : "";
      } else {
        return "";
      }
    },
    getCodeCategoryItems: (state) => (category) => {
      const findItems = state.codes.find((code) => code.category === category);
      return findItems !== undefined ? findItems.items : [];
    },
  },
  mutations: {
